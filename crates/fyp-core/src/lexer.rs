...
impl<'a> Lexer<'a> {
    ...
    pub fn at(input: &'a [u8], offset: usize) -> Self {
        Lexer { input, pos: offset.min(input.len()) }
    }
        ...
        match b {
            ...
            b'>' => {
                if self.peek_at(1) == Some(b'>') {
                    ...
                    Err(Error::Syntax { offset: start, message: "stray '>'".into() })
                }
            }
            b'(' => {
                self.pos += 1;
            ...
            b')' => Err(Error::Syntax { offset: start, message: "stray ')'".into() }),
    ...
    fn number(&mut self) -> Result<Token> {
        ...
        let s = std::str::from_utf8(text)
            .map_err(|_| Error::Syntax { offset: start, message: "non-ASCII number".into() })?;
        if !s.contains('.') {
        ...
        let cleaned = if cleaned.starts_with('.') { format!("0{cleaned}") } else { cleaned };
        let cleaned = if cleaned.ends_with('.') { format!("{cleaned}0") } else { cleaned };
        cleaned
            .parse::<f64>()
            .map(Token::Real)
            .map_err(|_| Error::Syntax { offset: start, message: format!("bad number {s:?}") })
    }
    ...
    fn keyword(&mut self) -> Result<Token> {
        ...
        if self.pos == start {
            ...
            self.pos += 1;
            return Err(Error::Syntax { offset: start, message: "unexpected delimiter".into() });
        }
    ...
    fn hex_string(&mut self) -> Result<Token> {
        ...
        loop {
            ...
            let Some(v) = hex_value(b) else {
                return Err(Error::Syntax { offset: start, message: "non-hex byte in hex string".into() });
            };
            match pending.take() {
                None => pending = Some(v),
...
mod tests {
    ...
    #[test]
    fn numbers() {
        assert_eq!(all(b"42 -17 +3"), vec![Token::Integer(42), Token::Integer(-17), Token::Integer(3)]);
        assert_eq!(all(b"3.5 -.002 .5 6."), vec![
            Token::Real(3.5),
            Token::Real(-0.002),
            Token::Real(0.5),
            Token::Real(6.0)
    ...
    fn names_with_escapes() {
        assert_eq!(all(b"/Type /A#20B /"), vec![
    ...
    #[test]
    fn strings() {
        assert_eq!(all(b"(hello (nested) \\n\\101)"), vec![Token::LiteralString(b"hello (nested) \nA".to_vec())]);
        assert_eq!(all(b"<48656C6C6F 7>"), vec![Token::HexString(b"Hello\x70".to_vec())]);
    }

    #[test]
    fn delimiters_and_keywords() {
        assert_eq!(all(b"<< /K [1 0 R] >> obj"), vec![
            Token::DictOpen,
