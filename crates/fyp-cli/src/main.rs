    ...
    match cli.command {
        Cmd::Info { path } => {
            ...
            if info.header_offset > 0 {
                println!("en-tête      décalé de {} octets (toléré)", info.header_offset);
            }
            match info.startxref {
                Some(off) => println!("startxref    {off}"),
                None => println!("startxref    absent (reconstruction nécessaire)"),
            }
            println!("chiffré      {}", if info.looks_encrypted { "probablement" } else { "non" });
            println!("%%EOF        {}", if info.has_eof_marker { "présent" } else { "absent" });
        }
        Cmd::Modules { dir, trusted } => {
            let (found, errors) = fyp_host::discover(&dir, trusted);
            for m in &found {
                let perms: Vec<String> = m.manifest.permissions.iter().map(|p| format!("{p:?}")).collect();
                println!(
                    "{:<24} {:<12} {:<8} {:?} [{}]",
                    m.manifest.id,
                    m.manifest.version,
            ...
            println!("{} module(s) accepté(s), {} refusé(s)", found.len(), errors.len());
        }
    }
    Ok(())
}
